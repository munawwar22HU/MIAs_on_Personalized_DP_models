...
class DPOptimizer(Optimizer):
    ...
    def clip_and_accumulate(self, pp_max_grad_norms: ndarray = None):
        ...
        if pp_max_grad_norms is not None:
            clip_threshold = torch.tensor(pp_max_grad_norms, device=per_sample_norms.device)
    ...
    #     per_sample_norms = torch.stack(per_param_norms, dim=1).norm(2, dim=1)
    #     clip_threshold = self.max_grad_norm
    #     if pp_max_grad_norms.any() is not None:
    #         clip_threshold = torch.tensor(pp_max_grad_norms, device=per_sample_norms.device)
    #     print(f"per_sample_norms device: {per_sample_norms.device}")
    #     print(f"clip_threshold device: {clip_threshold.device}")
    #     per_sample_clip_factor = (clip_threshold / (per_sample_norms + 1e-6))\
    #         .clamp(max=1.0)
    ...
    def pre_step(self, closure: Optional[Callable[[], float]] = None,
                 pp_max_grad_norms: ndarray = None) -> Optional[float]:
        """
        ...
        Args:
            ...
            current iteration.
        """
        self.clip_and_accumulate(pp_max_grad_norms)
    ...
    def step(self, closure: Optional[Callable[[], float]] = None,
        ...
        if self.pre_step(pp_max_grad_norms=pp_max_grad_norms):
